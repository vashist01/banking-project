...
public class AccountTransferService {
    ...
    public TransferResponse transfer(String senderAccountNumber, String receiverAccountNumber,
        ...
        try{
            ...
            BigDecimal transferFee = calculateTransferFee(senderAccountNumber,receiverAccountNumber,amount);
            BigDecimal totalDebit = amount.add(transferFee);
            BigDecimal maxWithDrawAmount = senderAccount.getAvailableBalance().add(senderAccount.getOverdraftLimit());
    ...
    private void releaseTransferLock(String senderAccountNumber, String receiverAccountNumber, String transferId) {
    ...
    private TransferResponse mapTransferResponse(Account senderAccount, String transferId, Account receiverAccount) {
        return TransferResponse.builder().transferId(transferId).amount(senderAccount.getBalance())
                .sourceNewBalance(senderAccount.getBalance()).destinationNewBalance(receiverAccount.getBalance()).
        sourceAccountId(senderAccount.getAccountNumber()).destinationAccountId(receiverAccount.getAccountNumber())
                .currency(senderAccount.getCurrency()).build();
    }


    private BigDecimal calculateTransferFee(String senderAccountNumber, String receiverAccountNumber, BigDecimal amount) {
        boolean internalTransfer = senderAccountNumber.startsWith("SV") && receiverAccountNumber.startsWith("SV");
        if(internalTransfer){
    ...
    private void acquireTransferLocks(String senderAccountNumber, String receiverAccountNumber, String transferId) {
        String lock1,lock2;
    ...
    private void acquireLock(String accountNumber, String transferId) {
        ...
        Boolean acquireLock = redisTemplate.opsForValue().setIfAbsent(accountNumber,transferId, Duration.ofDays(1));
    ...
    private void checkDailyTransferLimit(String senderAccountNumber, BigDecimal amount) {
        ...
        if (dailyTransferLimit == null) {
            DailyTransferLimitProjection dailyTransferLimitProjection = accountRepository.
                    findDailyTransferLimitByAccountNumber(senderAccountNumber);
            ...
            if (dailyTransferLimit.add(amount).compareTo(dailyTransferLimit) > 0) {
                throw new TransferValidationException(
                        "Daily transfer limit exceeded. Limit: " + dailyTransferLimit +
                                ", Used: " + dailyTransferLimit + ", Requested: " + amount
                );
            }
        }
    }
